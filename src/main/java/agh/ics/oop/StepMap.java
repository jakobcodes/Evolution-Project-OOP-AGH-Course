...
public class StepMap extends AbstractWorldMap { // Steppe
    ...
    @Override
    protected List<Animal> getAnimalsOnMap() {  // trzeba znaleźć sensowny kompromis między za niskim, a za wysokim poziomem abstrakcji: nie lepiej przeiterować po kluczach mapy animals?
        List<Animal> animals = new LinkedList<>();  // ta lista jest pobierana kilkukrotnie w ciągu rundy, a jest stosunkowo mało zmienna (narodziny/śmierć zwierzęcia) - może warto ją trzymać i tylko aktualizować w miarę potrzeby?
        int minX = getLeftBottomCorner().getX();
        int minY = getLeftBottomCorner().getY();
        int maxX = getRightTopCorner().getX();
        int maxY = getRightTopCorner().getY();
